...
namespace Img2table.Sharp.Img2table.Tables.Processing.BorderlessTables.layout
{
    public class TableSegments
    {
        ...
        {
            List<Whitespace> h_ws = Whitespaces.GetWhitespaces(segment, vertical: false, pct: 1, min_width: 0.5 * medianLineSep);
            ...
            for (int i = 0; i < h_ws.Count - 1; i++)
            {
                ...
                {
                    var v_ws = Whitespaces.GetRelevantVerticalWhitespaces(segArea, charLength, medianLineSep, pct: 0.66);
