...
namespace Img2table.Sharp.Core.Tables
{
    public static class Utils
    {
        ...
        {
            // Check if the sizes and types are the same
            ...
            // Check if there are any non-zero elements in the difference matrix
        ...
        {
            // Convert Mat to a one-dimensional array based on its type
        ...
        {
            // 创建一个与cc相同大小的Mat对象，类型为CV_8UC1
            Mat rsla_img = new Mat(mat.Size(), MatType.CV_8UC1);

            // 遍历cc矩阵，将大于0的元素设置为1，其他元素设置为0
            for (int row = 0; row < mat.Rows; row++)
            {
                for (int col = 0; col < mat.Cols; col++)
                {
