using img2table.sharp.Core.Tabular.Processing.BorderlessTables.Layout;
...
namespace Img2table.Sharp.Core.Tabular.Processing.BorderlessTables
{
    public class BorderlessTables
    {
        public static List<Object.Table> IdentifyBorderlessTables(Mat thresh, List<Line> lines, double char_length, 
            double medianLineSep, List<Cell> contours, List<Object.Table> existingTables)
        {
            var tableSegments = Layout.SegmentImage(thresh, lines, char_length, medianLineSep, existingTables);

            var tables = new List<Object.Table>();
        ...
        private static Object.Table CoherentTable(Object.Table tb, List<Cell> elements)
        {
            ...
            if (row_range.Count > 0)
            {
                ...
                if (newRows.Count >= 2)
                {
                    return new Object.Table(newRows, true);
                }
            }
        ...
        private static DataTable CreateDataFrame(Object.Table tb)
        {
        ...
        private static List<Object.Table> DeduplicateTables(List<Object.Table> identifiedTables, List<Object.Table> existingTables)
        {
            identifiedTables = identifiedTables.OrderByDescending(tb => tb.Area).ToList();

            List<Object.Table> finalTables = new List<Object.Table>();
